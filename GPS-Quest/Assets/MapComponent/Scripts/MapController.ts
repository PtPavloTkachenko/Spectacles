...
// PlaceListCreator removed for outdoor GPS navigation
import {TWEEN_DURATION} from "./MapUtils"
...
export class MapController extends BaseScriptComponent {
  ...
  // Place list creator removed for outdoor GPS navigation
  ...
  private currentActiveWaypointIndex: number = 0  // Track current waypoint in quest
  ...
  initialize(
    ...
    // Place list creator assignment removed for outdoor GPS navigation
  ...
  private onUpdate() {
    ...
    // Update all pins on the map
    this.pinSet.forEach((p) => {
      this.updatePinPositionOnMap(p)
      // GPS Quest Enhancement: Check and update visited state for each pin
      // Pins turn green when user approaches within activation distance
      p.updateVisitedState()
    })

    // GPS Quest Enhancement: Update waypoint visibility for step-by-step progression
  ...
  createMapPin(place: Place): MapPin {
    ...
    // GPS Quest Enhancement: Initialize visited state visualization
    // If waypoint was already visited, show it in green immediately
    if (place && place.visited) {
      pin.setVisited(true)
    }

    // GPS Quest Enhancement: Hide pins initially except the first one
    // Only show visited pins and the next active waypoint
    const pins = Array.from(this.pinSet)
    const isFirstPin = this.pinSet.size === 0  // Check if this is the first pin being added

    if (isFirstPin) {
      // First waypoint is always visible and active
      pin.setNextActive(true, false)
      log.i(`GPS Quest: First waypoint "${place.name}" is active`)
    } else {
      // Hide initially, will be shown when it becomes active
      pin.setNextActive(false, false)
      log.i(`GPS Quest: Waypoint "${place.name}" is hidden initially`)
    }
  ...
  configureCell(cell: Cell): void {
    ...
    if (cell.imageComponent && cell.imageComponent.mainMaterial && cell.imageComponent.mainMaterial.mainPass) {
      cell.imageComponent.mainMaterial.mainPass.baseTex = mapTexture
    }
  ...
  handleHoverUpdate(localPosition: vec2): void {
    ...
    // Place list creator hover removed for outdoor GPS navigation
  }
  ...
  /**
   * GPS Quest Enhancement: Updates waypoint visibility for step-by-step progression.
   * Shows only visited waypoints and the next active waypoint.
   * When a waypoint is visited, reveals the next one with animation.
   *
   * @private
   */
  private updateWaypointVisibility(): void {
    ...
    // Sort pins to ensure consistent order (you might want to add an index property)
    // For now, we'll use the order they were added
    let foundNextActive = false
    let lastVisitedIndex = -1

    // Find the last visited waypoint
    pins.forEach((pin, index) => {
      if (pin.place && pin.place.visited) {
        lastVisitedIndex = index
      }
    })

    // Update visibility for each pin
    pins.forEach((pin, index) => {
      if (pin.place) {
        const isVisited = pin.place.visited

        if (isVisited) {
          // Show visited waypoints (green)
          if (!pin.shouldBeVisible()) {
            pin.setNextActive(true, false)
          }
        } else if (!foundNextActive) {
          // This is the next active waypoint
          foundNextActive = true

          // Check if this waypoint just became active
          if (!pin.shouldBeVisible() && lastVisitedIndex === index - 1) {
            // Animate the appearance of the new active waypoint
            log.i(`GPS Quest: Revealing next waypoint - ${pin.place.name}`)
            pin.setNextActive(true, true)  // Animate appearance
          } else if (index === 0) {
            // First waypoint should always be visible
            pin.setNextActive(true, false)
          }
        } else {
          // Future waypoints - keep hidden
          if (pin.shouldBeVisible() && !isVisited) {
            pin.setNextActive(false, true)  // Animate disappearance
          }
        }
      }
    })

    // Update current active waypoint index
    this.currentActiveWaypointIndex = lastVisitedIndex + 1
  }
}
